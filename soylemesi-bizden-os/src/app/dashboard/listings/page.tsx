...
export default function SovereignDashboard() {
  ...
  // OMNI-SEARCH VE FİLTRE MATRİSİ
  const [searchQuery, setSearchQuery] = useState('');
  const [showFilters, setShowFilters] = useState(false);
  const [advancedFilters, setAdvancedFilters] = useState({ minPrice: '', maxPrice: '', minArea: '', maxArea: '' });
  ...
  const [formData, setFormData] = useState({ 
    title: '', transactionType: 'SALE' as any, subCategory: 'Daire', city: '', district: '', 
  ...
  const handleDeleteAsset = async (id: string) => {
    if(!confirm("İmha edilsin mi?")) return;
    setListings(prev => prev.filter(item => item.id !== id));
    setSelectedAsset(null);
    try { await fetch(`http://localhost:4000/listings/${id}`, { method: 'DELETE' }); } catch (e) {}
  };

  const openCommandCenter = (item: any) => {
  ...
  const calculateROI = (price: any, rentPrice: any) => {
    if (!price || !rentPrice || Number(price) === 0 || Number(rentPrice) === 0) return null;
  ...
  const filteredListings = listings.filter((item: any) => {
    if (activeTab !== 'ALL') { if (item.transactionType !== activeTab && item.transactionType !== 'BOTH') return false; }
    if (searchQuery) {
      const query = searchQuery.toLowerCase();
      const matchTitle = item.title?.toLowerCase().includes(query);
      const matchCity = item.city?.toLowerCase().includes(query);
      const matchAda = item.ada?.includes(query);
      if (!matchTitle && !matchCity && !matchAda) return false;
    }
    if (advancedFilters.minPrice && Number(item.price) < Number(advancedFilters.minPrice)) return false;
    if (advancedFilters.maxPrice && Number(item.price) > Number(advancedFilters.maxPrice)) return false;
    if (advancedFilters.minArea && Number(item.area) < Number(advancedFilters.minArea)) return false;
    if (advancedFilters.maxArea && Number(item.area) > Number(advancedFilters.maxArea)) return false;
    return true;
  });

  const totalValue = filteredListings.reduce((s, i: any) => s + (Number(i.price) || 0), 0);
  const totalArea = filteredListings.reduce((s,i:any)=> s + (Number(i.area) || 0), 0);
  const avgUnitPrice = totalValue / (totalArea || 1);

  const calculateScore = (price: number, area: number) => {
    if (!price || !area || avgUnitPrice === 0) return { label: 'ANALİZ EDİLİYOR', color: 'text-slate-400', bg: 'bg-slate-50' };
    const unit = price / area;
    if (unit < avgUnitPrice * 0.8) return { label: 'YÜKSEK FIRSAT', color: 'text-green-600', bg: 'bg-green-50' };
    if (unit > avgUnitPrice * 1.2) return { label: 'PREMIUM VARLIK', color: 'text-orange-600', bg: 'bg-orange-50' };
    return { label: 'PAZAR DENGESİ', color: 'text-blue-600', bg: 'bg-blue-50' };
  };

  // YENİ: VARLIK DAĞILIMI HESAPLAMALARI (ASSET ALLOCATION)
  const assetTypes = filteredListings.reduce((acc: any, item: any) => {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-black text-blue-600 text-4xl animate-pulse italic tracking-[0.5em]">SOVEREIGN IQ BOOTING...</div>;

  return (
    <div className="min-h-screen bg-[#F8FAFC] p-4 lg:p-8 font-sans text-slate-900 selection:bg-blue-600 selection:text-white">
      <div className="max-w-[1850px] mx-auto bg-white shadow-2xl rounded-[3rem] lg:rounded-[5rem] p-6 lg:p-16 border border-slate-100 relative overflow-hidden">
        ...
        <header className="mb-10 border-b border-slate-50 pb-10 flex flex-col xl:flex-row justify-between items-center gap-8 relative z-30">
          ...
          <div className="flex-1 flex justify-center xl:justify-end gap-4 items-center">
            ...
            <div className="flex bg-slate-50 p-1.5 rounded-2xl border border-slate-100 shadow-inner">
               {['TRY', 'USD', 'EUR'].map((c: any) => (
                 <button key={c} onClick={() => setCurrency(c)} className={`px-8 py-3 rounded-xl text-[10px] font-black transition-all ${currency === c ? 'bg-white text-blue-600 shadow-xl' : 'text-slate-400'}`}>{c}</button>
               ))}
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-12 relative z-10">
          <div className="xl:col-span-3 space-y-8">
            ...
            {/* 📊 YENİ: VARLIK DAĞILIMI (ASSET ALLOCATION BAR) VE KPI MATRIX */}
               ...
               <div className="flex items-center gap-2">
                 <div className="flex bg-slate-50 p-1.5 rounded-full border border-slate-100">
                    {['ALL', 'SALE', 'RENT'].map(tab => (
                      <button key={tab} onClick={() => setActiveTab(tab as any)} className={`px-6 py-2.5 rounded-full text-[9px] font-black tracking-widest transition-all ${activeTab === tab ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>{tab}</button>
                    ))}
                 </div>
            ...
            {showFilters && (
               <div className="bg-slate-900 rounded-[3rem] p-8 border border-slate-800 shadow-2xl animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-h-[800px] overflow-y-auto pr-4 custom-scrollbar">
              {filteredListings.length === 0 ? (
                ...
                filteredListings.map((item: any) => {
                  ...
                  return (
                    <div key={item.id} onClick={() => openCommandCenter(item)} className="bg-white rounded-[4rem] p-10 border border-slate-100 shadow-xl hover:shadow-2xl transition-all group border-t-8 border-t-blue-600 relative overflow-hidden cursor-pointer">
              ...
              <form onSubmit={handleAddAsset} className="space-y-5">
                ...
                <div className="grid grid-cols-1 gap-3">
                   {formData.transactionType !== 'RENT' && (
                     <input type="number" required placeholder="Satış Fiyatı (TRY)" className="w-full bg-slate-800 text-blue-400 font-black p-5 rounded-xl text-sm outline-none border border-slate-700 shadow-inner" value={formData.price} onChange={(e)=>setFormData({...formData, price: e.target.value})} />
                   )}
                   {formData.transactionType !== 'SALE' && (
                     <input type="number" placeholder="Aylık Kira Getirisi (TRY)" className="w-full bg-slate-800 text-orange-400 font-black p-5 rounded-xl text-sm outline-none border border-slate-700 shadow-inner" value={formData.rentPrice} onChange={(e)=>setFormData({...formData, rentPrice: e.target.value})} />
                   )}
                </div>
        ...
        {/* 🎬 7. COMMAND CENTER MODAL */}
        {selectedAsset && (
          <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-2xl z-[100] flex items-center justify-center p-6 overflow-y-auto">
              ...
              <div className="p-12 lg:p-20">
                 <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-slate-100 pb-10 mb-10 gap-6">
                    <div className="flex-1 w-full">
                      ...
                      {isEditMode ? (
                        <input type="text" className="w-full text-5xl lg:text-6xl font-black tracking-tighter text-slate-900 border-b-2 border-blue-500 focus:outline-none bg-slate-50 rounded-xl px-4 py-2" value={editFormData.title} onChange={(e) => setEditFormData({...editFormData, title: e.target.value})} />
                      ) : (
                        <h2 className="text-5xl lg:text-6xl font-black tracking-tighter text-slate-900 mb-2">{selectedAsset.title}</h2>
                      )}
                    ...
                    <div className="text-left md:text-right">
                      {isEditMode ? (
                        ...
                        <p className="text-5xl lg:text-6xl font-black text-blue-600 tracking-tighter">{formatValue(selectedAsset.price)}</p>
                      )}
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Verified Market Price</p>
                    </div>
                 </div>

                 {/* VISUAL ENGINE & BLUEPRINT MODE */}
                 <div className="mb-12">
                    <div className="w-full h-[400px] bg-slate-900 rounded-[3rem] relative overflow-hidden group flex flex-col shadow-inner transition-all duration-500">
                       ...
                       <div className="h-20 bg-slate-950/90 backdrop-blur-md border-t border-white/10 flex items-center justify-between px-8">
                          ...
                          <div className="flex gap-3">
                             {[0, 1, 2, 3].map((idx) => (
                                <button key={idx} onClick={() => { setActiveImageIdx(idx); setIsMeasureMode(false); }} className={`w-12 h-10 rounded-lg border-2 transition-all ${activeImageIdx === idx && !isMeasureMode ? 'border-blue-500 bg-blue-500/20' : 'border-white/10 bg-white/5 hover:border-white/30'}`}></button>
                             ))}
                          </div>
                       </div>
                    </div>
                 ...
                 {/* FINANCIAL ENGINE (ROI & AMORTİSMAN) */}
                 {calculateROI(selectedAsset.price, selectedAsset.rentPrice) && (
                 ...
                 <div className="mt-12 flex justify-end gap-6 items-center border-t border-slate-100 pt-8">
                    {isEditMode ? (
                       <button onClick={handleUpdateAsset} className="bg-green-600 text-white px-16 py-6 rounded-full font-black uppercase text-xs tracking-[0.3em] shadow-2xl hover:scale-105 transition-all">Mühürle (Kaydet)</button>
                    ) : (
                       <>
        ...
        {/* 📑 8. EXECUTIVE REPORT MODAL */}
        {isGeneratingReport && (
          <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-3xl z-[150] flex items-center justify-center p-6">
