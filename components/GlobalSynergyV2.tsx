...
import { Share2, Globe, Activity, ShieldCheck, Database, Zap } from 'lucide-react';

export const GlobalSynergyV2 = () => {
  const synergyNodes = [
    { node: 'REGISTRY_SYNC', load: '12%', status: 'SYNCHRONIZED' },
    { node: 'MARKET_DATA_HUB', load: '45%', status: 'REALTIME' },
    { node: 'LEGAL_DATABASE', load: '08%', status: 'SECURED' }
  ];
    ...
    <div style={{ padding: '40px', backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: '32px', position: 'relative', overflow: 'hidden' }}>
      <header style={{ marginBottom: '35px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--accent-emerald)', fontWeight: '950', fontSize: '0.7rem', letterSpacing: '3px' }}>
            <Share2 size={18} /> GLOBAL SYNERGY v2
          </div>
          <h3 style={{ fontSize: '1.5rem', fontWeight: '950', marginTop: '10px' }}>Egemen Veri Bağlantısı</h3>
        </div>
        ...
        {synergyNodes.map((n, i) => (
          <div key={i} style={{ padding: '20px', backgroundColor: 'var(--bg-primary)', borderRadius: '20px', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ fontSize: '0.75rem', fontWeight: '900', color: 'var(--text-secondary)' }}>{n.node}</span>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '0.85rem', fontWeight: '950' }}>{n.status}</div>
              <div style={{ fontSize: '0.55rem', fontWeight: '950', color: 'var(--accent-emerald)' }}>LOAD: {n.load}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding: '20px', backgroundColor: '#FFF', borderRadius: '15px', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <span style={{ fontSize: '0.75rem', fontWeight: '900' }}>DATA INTEGRITY: 100%</span>
        </div>
      ...
      <div style={{ marginTop: '25px', textAlign: 'center' }}>
        <p style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', fontWeight: '600' }}>
          {"Sinerji Akısı: $$ \\Phi_{sync} = \\oint_{\\mathcal{S}} \\vec{V}_{data} \\cdot d\\vec{A} \\oplus \\text{Nexus}_{key} $$ mühürlenmiştir."}
        </p>
      </div>
    </div>
  );
