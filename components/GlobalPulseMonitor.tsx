...
import { Globe, Activity, TrendingUp, Zap, ArrowUpRight } from 'lucide-react';

export const GlobalPulseMonitor = () => {
  const indices = [
    { label: 'GLOBAL LIQUIDITY INDEX', val: '412.5', change: '+1.2%', status: 'Bullish' },
    { label: 'CARBON CREDIT PRICE (ECQ)', val: '€ 84.20', change: '+4.8%', status: 'Extreme' },
    { label: 'INTEREST RATE FORECAST', val: '%4.25', change: '-0.1%', status: 'Stable' }
  ];
    ...
    <div style={{ padding: '40px', backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: '28px' }}>
      <header style={{ marginBottom: '35px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--accent-emerald)', fontWeight: '950', fontSize: '0.7rem', letterSpacing: '3px' }}>
          <Globe size={18} /> GLOBAL INTELLIGENCE FEED
        </div>
      ...
      <div style={{ marginTop: '30px', padding: '15px', backgroundColor: 'rgba(26,188,156,0.05)', borderRadius: '12px', textAlign: 'center' }}>
        <p style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: '600', margin: 0 }}>
          {"Makro Sentez: $$ \Omega_{sync} = \int_{t-n}^{t} \frac{\Delta Asset}{\Delta Global} dt $$ mühürlenmiştir."}
        </p>
      </div>
    </div>
  );
