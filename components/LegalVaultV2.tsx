...
import { ShieldAlert, FileLock, SearchCheck, Gavel, Key, Activity, Fingerprint } from 'lucide-react';

export const LegalVaultV2 = () => {
  const vaultStatus = [
    { label: 'TAPU BELGELERİ', status: 'SHARDED_ENCRYPTION', integrity: '100%' },
    { label: 'SÖZLEŞME ARŞİVİ', status: 'TIME_LOCKED', integrity: 'SECURED' },
    { label: 'ADLİ TARSAMA', status: 'CLEAN', integrity: 'VERIFIED' }
  ];
    ...
    <div style={{ padding: '40px', backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: '32px', position: 'relative', overflow: 'hidden' }}>
      <header style={{ marginBottom: '35px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--accent-emerald)', fontWeight: '950', fontSize: '0.7rem', letterSpacing: '3px' }}>
            <FileLock size={18} /> LEGAL VAULT v2
          </div>
          <h3 style={{ fontSize: '1.5rem', fontWeight: '950', marginTop: '10px' }}>Hukuki Kasa & Arşiv</h3>
        </div>
        ...
        {vaultStatus.map((item, i) => (
          <div key={i} style={{ padding: '20px', backgroundColor: 'var(--bg-primary)', borderRadius: '20px', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '0.85rem', fontWeight: '950' }}>{item.status}</div>
              <div style={{ fontSize: '0.55rem', fontWeight: '950', color: 'var(--accent-emerald)' }}>{item.integrity}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding: '20px', backgroundColor: '#FFF', borderRadius: '15px', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <SearchCheck size={18} color="var(--accent-emerald)" />
          <span style={{ fontSize: '0.75rem', fontWeight: '900' }}>ADLİ ANALİZ MOTORU: AKTİF</span>
        </div>
      ...
      <div style={{ marginTop: '25px', textAlign: 'center' }}>
        <p style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', fontWeight: '600' }}>
          {"Gerçeklik Kanıtı: $$ \\Psi_{truth} = \\mathcal{H}(\\text{Doc} \\oplus \\text{Key}_{bio}) \\equiv \\text{Immutable} $$ mühürlenmiştir."}
        </p>
      </div>
    </div>
  );
